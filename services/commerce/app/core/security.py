"""Access-token verification (COM-102).

Commerce is a pure *resource server*: it never issues tokens, it only verifies the RS256 access
tokens minted by the Identity service. Verification keys are resolved from Identity's published
JWKS (``/.well-known/jwks.json``) by the token's ``kid`` header, so key rotation stays a deploy-time
concern of Identity alone.

The verifier depends on an abstract :class:`SigningKeyResolver` (satisfied in production by PyJWT's
``PyJWKClient`` and in tests by a static double), so the verification logic is unit-testable without
any network access. This mirrors the Dietary service so both resource servers behave identically.
"""
