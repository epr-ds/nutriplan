"""COM-404/405: the grocery adapter factory selects a real adapter vs the fake by configuration.

Guards the COM-905 credential seam: FreshBasket (COM-404) and Walmart (COM-405) are each backed by
their real HTTP adapter only when a sandbox API key is configured, and by the in-process fake
otherwise (dev/CI), while every other provider stays on the fake for now.
"""
